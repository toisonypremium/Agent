...
function veVon(x){
 ...
 window.__vonVanHanh=c;const ds=c.assets||[];$('#danh-sach-von').innerHTML=ds.length?ds.map(a=>`<div class="the-tai-san"><div class="hang-tai-san"><div><strong>${html(a.symbol)}</strong><small>${html(dich(a.state))} · sẵn sàng ${phanTram(a.readiness)}</small></div><span class="huy-hieu">${phanTram(a.target_allocation_pct)}</span></div><div class="thanh-tien-do"><i style="width:${Math.min(100,(a.existing_exposure_usdt||0)/(a.strategic_cap_usdt||1)*100)}%"></i></div><div class="luoi-nho"><div><span>Trần chiến lược</span><b>${tien(a.strategic_cap_usdt)} USDT</b></div><div><span>Đã sử dụng</span><b>${tien(a.existing_exposure_usdt)} USDT</b></div><div><span>Dư địa</span><b>${tien(a.remaining_strategic_usdt)} USDT</b></div><div><span>Giải ngân ngay</span><b>${tien(a.executable_budget_usdt)} USDT</b></div></div><p class="ly-do">${html(a.reason||a.next_trigger||'—')}</p></div>`).join(''):'<div class="rong">Chưa có dữ liệu phân bổ</div>';
}
function veThiTruong(x,tx){
 ...
 const sc=baoCao(tx,'bang_diem_ky_thuat'), coins=sc.coins||[];$('#danh-sach-tai-san').innerHTML=coins.length?coins.map(a=>`<div class="the-tai-san"><div class="hang-tai-san"><div><strong>${html(a.symbol)}</strong><small>${html(dich(a.state))} · ${html(dich(a.verdict))}</small></div><span class="huy-hieu">${so(a.technical_score*100,0)} điểm</span></div><div class="thanh-tien-do"><i style="width:${Math.max(0,Math.min(100,a.technical_score*100))}%"></i></div><div class="luoi-nho"><div><span>Hạng luân chuyển</span><b>#${a.rotation_rank||'—'}</b></div><div><span>Thanh khoản</span><b>${html(a.liquidity_grade||'—')} · ${so(a.liquidity_score,0)}</b></div><div><span>Tỷ lệ lợi nhuận/rủi ro</span><b>${so(a.reward_risk,2)}</b></div><div><span>Khoảng cách vùng mua</span><b>${so(a.discount_gap_pct*100,2)}%</b></div></div><p class="ly-do"><strong>Điểm chặn:</strong> ${html(a.top_blocker||'Không có')}</p><p class="ly-do"><strong>Kích hoạt tiếp:</strong> ${html(a.next_trigger||'—')}</p></div>`).join(''):'<div class="rong">Chưa có dữ liệu tài sản</div>';
}
function veViThe(x){
 const d=x.du_lieu||{}, ps=d.vi_the||[], os=d.lenh_dang_mo||[];gan('#so-vi-the',`${ps.length} VỊ THẾ`);gan('#so-lenh',`${os.length} LỆNH`);
 $('#danh-sach-vi-the').innerHTML=ps.length?ps.map(p=>`<div class="the-tai-san"><div class="hang-tai-san"><div><strong>${html(p.Symbol||p.symbol||'—')}</strong><small>${html(p.InstID||p.inst_id||'')}</small></div><span class="huy-hieu dat">ĐANG NẮM GIỮ</span></div><div class="luoi-nho"><div><span>Số lượng</span><b>${so(p.Quantity??p.quantity,8)}</b></div><div><span>Giá vốn</span><b>${tien(p.AvgEntryPrice??p.avg_entry_price)}</b></div><div><span>Giá trị vốn</span><b>${tien(p.CostBasis??p.cost_basis)} USDT</b></div><div><span>Cập nhật</span><b>${tg((p.UpdatedAt??p.updated_at)*1000)}</b></div></div></div>`).join(''):'<div class="rong">Không có vị thế</div>';
 $('#danh-sach-lenh').innerHTML=os.length?os.map(o=>`<div class="the-tai-san"><div class="hang-tai-san"><strong>${html(o.Symbol||o.symbol||'—')}</strong><span class="huy-hieu">${html(dich(o.Status||o.status))}</span></div><p class="ly-do">${html(o.Side||o.side||'—')} · ${html(o.Type||o.type||'—')} · ${tien(o.Notional||o.notional)} USDT</p></div>`).join(''):'<div class="rong">Không có lệnh live đang mở</div>';
}
...
setInterval(()=>{if(!document.hidden)nap()},30000);

async function napHermes(){try{const x=await api('/api/v1/hermes');veHermes(x)}catch{gan('#hermes-van-hanh','Không đọc được dữ liệu')}}
function veHermes(x){
 const d=x.du_lieu||{}, hr=d.bao_cao_quan_ly?.du_lieu||{}, sd=d.quyet_dinh_bong?.du_lieu||{}, sv=d.bo_giam_sat?.du_lieu||{}, bs=d.trang_thai_bot?.du_lieu||{}, ev=d.bang_chung_thuc_thi?.du_lieu||{}, lp=d.bao_cao_vi_the?.du_lieu||{}; const warnings=d.canh_bao_do_tuoi||[]; const warningEl=$('#hermes-canh-bao'); if(warningEl){warningEl.hidden=!(warnings.length||d.ghi_chu_lich); warningEl.classList.toggle('thong-tin',!warnings.length); warningEl.textContent=warnings.length?warnings.join(' · '):d.ghi_chu_lich;}
 const tuDong=sd.mode==='autonomous';gan('#hermes-che-do',tuDong?'TỰ ĐỘNG':'QUAN SÁT');$('#hermes-che-do').className=`huy-hieu ${tuDong?'dat':''}`;gan('#hermes-van-hanh',tuDong?'Tự động trong giới hạn policy':'Chỉ quan sát');gan('#hermes-phan-quyet',(hr.gate_summary||'—').replace('APPROVED_DRY_RUN','Được duyệt chạy thử').replace('DOCTOR_OK','Hệ thống đạt'));gan('#hermes-hanh-dong',(hr.action_line||sv.action||'—').replace('READ_ONLY — no order placed.','Chỉ đọc — không đặt lệnh.').replace('managed_cycle','Chu kỳ quản lý'));gan('#hermes-loi',so(sv.consecutive_errors,0));
 const qd=sd.validation?.decision||{};gan('#hermes-quyet-dinh',qd.decision_id?`${qd.decision_id.slice(0,18)}…`:'Không có');gan('#hermes-cap-nhat',tg(sd.generated_at));
 const op=window.__vonVanHanh||{};gan('#hermes-muc-tieu',phanTram(op.hermes_target_deployment_pct));gan('#hermes-da-dung',`${tien(op.already_committed_usdt)} USDT`);gan('#hermes-dung-luong',`${tien(op.hermes_conditional_capacity_usdt)} USDT`);gan('#hermes-von-state',op.hermes_capacity_state==='CAPACITY_AVAILABLE'?'CÒN DUNG LƯỢNG':'GIỚI HẠN');
 const dd=sv.portfolio_risk||{};gan('#hermes-sut-giam',dd.known?`${so((dd.drawdown_pct||0)*100,2)}%`:'Theo dõi qua ledger');gan('#hermes-bang-chung',dich(ev.status||'—'));
 const ps=lp.positions||[];$('#hermes-vi-the').innerHTML=ps.length?ps.map(p=>`<div class="the-tai-san"><div class="hang-tai-san"><div><strong>${html(p.symbol||p.Symbol||'—')}</strong><small>${html(p.inst_id||p.InstID||'')}</small></div><span class="huy-hieu">HERMES THEO DÕI</span></div><div class="luoi-nho"><div><span>Số lượng</span><b>${so(p.quantity??p.Quantity,8)}</b></div><div><span>Giá vốn</span><b>${tien(p.avg_entry_price??p.AvgEntryPrice)}</b></div><div><span>Giá trị vốn</span><b>${tien(p.cost_basis??p.CostBasis)} USDT</b></div><div><span>Trạng thái thoát</span><b>Đang giữ</b></div></div></div>`).join(''):'<div class="rong">Hermes chưa có vị thế để theo dõi</div>';
}
const napGoc=nap;nap=async function(){await napGoc();await napHermes()};
