...
func TestReadOnlyRoutesAndSecurityHeaders(t *testing.T) {
	api := testAPI(t)
	for _, path := range []string{"/healthz", "/api/v1/overview", "/api/v1/paper/scorecard", "/api/v1/paper/orders?limit=1", "/api/v1/events?limit=1", "/api/v1/audit?limit=1", "/api/v1/reports", "/api/v1/operations/health", "/api/v1/capital/overview", "/api/v1/assets/okx", "/api/v1/assets/okx/reconciliation", "/api/v1/strategy/dca", "/api/v1/dca/allocation", "/api/v1/market-planner", "/api/v1/capital/theses?limit=1"} {
...
func TestHaltIsUnavailableWithoutConfiguredAccess(t *testing.T) {
	api := testAPI(t)
	r := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/halt", strings.NewReader(`{"reason_code":"exchange_state","summary":"unsafe exchange state"}`))
