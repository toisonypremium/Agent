...
func TestExecuteHermesCanaryProbeRequiresDeterministicGates(t *testing.T) {
	...
	cfg.Live.LiveAutoMode, cfg.Live.SupervisorEnabled, cfg.Live.OrderManagementEnabled = true, true, true
	...
	result := ExecuteHermesDesiredOrders(context.Background(), cfg, agent2.Plan{State: agent2.StateScout}, []ManagedDesiredOrder{d}, nil, placer, recorder, ManagedExecutionContext{BTCAccumulationPhase: "MARKDOWN", HermesMode: "canary"}, false)
	if len(placer.calls) != 0 || len(result.Blocked) != 1 {
		t.Fatalf("unsafe Scout probe reached exchange: %+v", result)
	}
	placer, recorder = &hermesTestPlacer{}, &hermesTestRecorder{}
...
func TestExecuteHermesFirstOrderRequiresApprovedDryRun(t *testing.T) {
	cfg := cancelCfg()
	cfg.Live.FirstOrderRequireDryRun = true
	cfg.Live.MaxLiveNotionalPerOrderUSDT, cfg.Live.MaxLiveNotionalPerAssetUSDT, cfg.Live.MaxLiveNotionalTotalUSDT = 10, 20, 20
	cfg.Risk.NoFutures, cfg.Risk.NoLeverage, cfg.Risk.SpotLimitOnly = true, true, true
	d := ManagedDesiredOrder{Symbol: "RENDERUSDT", InstID: "RENDER-USDT", LayerIndex: 1, Side: "BUY", Type: "limit", Price: 1.5, Quantity: 2, Notional: 3, PostOnly: true, Source: "HERMES_OPERATOR", DecisionID: "decision-approval", Intent: "PROBE_LIMIT", AllocationTier: string(OpportunityProbe)}
	placer, recorder := &hermesTestPlacer{}, &hermesTestRecorder{}
	ctx := hermesExecutableContext()
	ctx.FirstOrderDryRunApproved = false
	blocked := ExecuteHermesDesiredOrders(context.Background(), cfg, hermesExecutablePlan(), []ManagedDesiredOrder{d}, nil, placer, recorder, ctx, false)
	if len(placer.calls) != 0 || len(blocked.Blocked) != 1 {
		t.Fatalf("missing approval reached exchange: %+v", blocked)
	}
	ctx.FirstOrderDryRunApproved = true
	allowed := ExecuteHermesDesiredOrders(context.Background(), cfg, hermesExecutablePlan(), []ManagedDesiredOrder{d}, nil, placer, recorder, ctx, false)
	if len(allowed.Placed) != 1 || len(placer.calls) != 1 {
		t.Fatalf("approved order blocked: %+v", allowed)
	}
}

func TestExecuteHermesDryRunNeverCallsExchange(t *testing.T) {
	cfg := cancelCfg()
	cfg.Live.MaxLiveNotionalPerOrderUSDT, cfg.Live.MaxLiveNotionalPerAssetUSDT, cfg.Live.MaxLiveNotionalTotalUSDT = 10, 20, 20
	cfg.Risk.NoFutures, cfg.Risk.NoLeverage, cfg.Risk.SpotLimitOnly = true, true, true
	d := ManagedDesiredOrder{Symbol: "RENDERUSDT", InstID: "RENDER-USDT", LayerIndex: 1, Side: "BUY", Type: "limit", Price: 1.5, Quantity: 2, Notional: 3, PostOnly: true, Source: "HERMES_OPERATOR", DecisionID: "decision-dry", Intent: "PROBE_LIMIT", AllocationTier: string(OpportunityProbe)}
	placer, recorder := &hermesTestPlacer{}, &hermesTestRecorder{}
	result := ExecuteHermesDesiredOrders(context.Background(), cfg, hermesExecutablePlan(), []ManagedDesiredOrder{d}, nil, placer, recorder, hermesExecutableContext(), true)
	if result.Status != ManagedCycleDryRun || len(result.Placed) != 1 || len(placer.calls) != 0 || len(recorder.reserved) != 0 {
		t.Fatalf("dry-run touched exchange/storage: %+v", result)
	}
}
...
func cancelCfg() config.Config {
	...
	c.Live.LiveAutoMode = true
	c.Live.SupervisorEnabled = true
	c.Live.OrderManagementEnabled = true
