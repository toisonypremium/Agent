...
func run(ctx context.Context, args []string) error {
	...
	if cmd == "config-check" {
		fmt.Printf("config_check=PASS path=%s mode=%s paper_trading=%t real_trading_enabled=%t\n", cfgPath, cfg.App.Mode, cfg.Execution.PaperTrading, cfg.Execution.RealTradingEnabled)
		return nil
	}
	...
	case "canary-rehearsal":
		return runCanaryRehearsal(ctx, cfg)
	...
	case "event-replay":
		r, err := db.ReplayHermesState()
		if err != nil {
			return err
		}
		fmt.Printf("events=%d checksum=%s positions=%+v\n", r.Events, r.Checksum, r.Positions)
		return nil
