...
func TestEvaluateCanaryReadinessBlocksEveryControl(t *testing.T) {
	...
	if r.Verdict != CanaryBlocked || len(r.Blockers) < 9 {
		t.Fatalf("expected all controls blocked: %+v", r)
	}
}

func TestValidateCanaryReadinessReportRequiresFreshCompleteEvidence(t *testing.T) {
	now := time.Now().UTC()
	report := CanaryReadinessResult{GeneratedAt: now, Verdict: CanaryReady, QualificationPassed: true, StressPassed: 100, StressRequired: 100, QualificationArtifact: "reports/qualification.json", QualificationSHA256: "abc123", DoctorStatus: DoctorOK, ReconcileStatus: ReconcileClean, ExecutionAuthority: true}
	if blockers := ValidateCanaryReadinessReport(report, now); len(blockers) != 0 {
		t.Fatalf("fresh report blocked: %v", blockers)
	}
