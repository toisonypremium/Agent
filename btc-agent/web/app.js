...
function veHermes(x){
 ...
 const ps=lp.positions||[];$('#hermes-vi-the').innerHTML=ps.length?ps.map(p=>`<div class="the-tai-san"><div class="hang-tai-san"><div><strong>${p.symbol||p.Symbol||'—'}</strong><small>${p.inst_id||p.InstID||''}</small></div><span class="huy-hieu">HERMES THEO DÕI</span></div><div class="luoi-nho"><div><span>Số lượng</span><b>${so(p.quantity??p.Quantity,8)}</b></div><div><span>Giá vốn</span><b>${tien(p.avg_entry_price??p.AvgEntryPrice)}</b></div><div><span>Giá trị vốn</span><b>${tien(p.cost_basis??p.CostBasis)} USDT</b></div><div><span>Trạng thái thoát</span><b>Đang giữ</b></div></div></div>`).join(''):'<div class="rong">Hermes chưa có vị thế để theo dõi</div>';
}
