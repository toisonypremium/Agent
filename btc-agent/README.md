...
- Code hỗ trợ production `live-auto`, nhưng quyền gửi lệnh thật là trạng thái runtime do operator kiểm soát. Audit gần nhất chỉ phê duyệt halted/canary-safe operation; chưa phê duyệt unrestricted live trading. Mỗi lệnh vẫn fail-closed qua market authority, audit/proof, final assertion, risk, reconcile, liquidity và notional caps; không gate nào được nới.
...
Production hiện chạy không có `--dry-run`, nhưng điều đó không tự cấp quyền gửi lệnh. Nếu bất kỳ gate nào fail, bot block hoặc chỉ reconcile; lệnh thật chỉ được gửi khi current market authority, audit/proof, final assertion và toàn bộ live guard cùng pass. `APPROVED_MONITORING` hoặc `APPROVED_DRY_RUN` không phải quyền gửi lệnh thật.
