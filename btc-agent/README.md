...
- Trạng thái được duyệt hiện tại: monitoring + supervisor dry-run. Real `dry_run=false` chỉ bật sau khi `live-auto-audit` trả `APPROVED_REAL_ORDER`.
...
Nếu bất kỳ gate nào fail, bot block hoặc chỉ reconcile. Chỉ bỏ `--dry-run` sau khi `live-auto-audit` trả `APPROVED_REAL_ORDER`; `APPROVED_DRY_RUN` chưa phải quyền gửi lệnh thật.
