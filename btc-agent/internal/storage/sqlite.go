...
func (d *DB) Migrate() error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS verified_usdt_observations(observation_key TEXT PRIMARY KEY, available_usdt REAL NOT NULL CHECK(available_usdt >= 0), observed_at INTEGER NOT NULL, payload_json TEXT NOT NULL);`,
		`CREATE INDEX IF NOT EXISTS idx_verified_usdt_observed_at ON verified_usdt_observations(observed_at DESC);`,
		`CREATE TABLE IF NOT EXISTS dca_allocation_epochs(id INTEGER PRIMARY KEY AUTOINCREMENT, idempotency_key TEXT NOT NULL UNIQUE, observed_available_usdt REAL NOT NULL CHECK(observed_available_usdt >= 0), envelope_usdt REAL NOT NULL CHECK(envelope_usdt >= 0), net_new_usdt REAL NOT NULL CHECK(net_new_usdt >= 0), observed_at INTEGER NOT NULL, created_at INTEGER NOT NULL, payload_json TEXT NOT NULL);`,
		`CREATE TABLE IF NOT EXISTS dca_allocation_entry_applications(epoch_id INTEGER NOT NULL REFERENCES dca_allocation_epochs(id), thesis_id TEXT NOT NULL, applied_at INTEGER NOT NULL, PRIMARY KEY(epoch_id, thesis_id));`,
		`CREATE TABLE IF NOT EXISTS dca_allocation_entries(epoch_id INTEGER NOT NULL REFERENCES dca_allocation_epochs(id), thesis_id TEXT NOT NULL, symbol TEXT NOT NULL, allocation_ratio REAL NOT NULL CHECK(allocation_ratio >= 0), amount_usdt REAL NOT NULL CHECK(amount_usdt >= 0), PRIMARY KEY(epoch_id, thesis_id));`,
		`CREATE TABLE IF NOT EXISTS dca_execution_state(singleton INTEGER PRIMARY KEY CHECK(singleton=1), global_cap_percent REAL NOT NULL CHECK(global_cap_percent >= 20 AND global_cap_percent <= 100), updated_at INTEGER NOT NULL);`,
		`CREATE TABLE IF NOT EXISTS dca_global_cap_events(client_order_id TEXT PRIMARY KEY, previous_cap_percent REAL NOT NULL, next_cap_percent REAL NOT NULL, created_at INTEGER NOT NULL);`,
		`CREATE TABLE IF NOT EXISTS dca_safety_state(singleton INTEGER PRIMARY KEY CHECK(singleton=1), consecutive_errors INTEGER NOT NULL DEFAULT 0, stale_observer_epochs INTEGER NOT NULL DEFAULT 0, auto_halt_reason TEXT NOT NULL DEFAULT '', updated_at INTEGER NOT NULL);`,
		`CREATE INDEX IF NOT EXISTS idx_dca_allocation_epochs_observed_at ON dca_allocation_epochs(observed_at DESC);`,
