...
func TestApplyDCAAllocationEpochToThesesCreatesAndReplaysLedgers(t *testing.T) {
	db, err := Open(filepath.Join(t.TempDir(), "allocation.sqlite")); if err != nil { t.Fatal(err) }; defer db.Close()
	...
	applied, err := db.ApplyDCAAllocationEpochToTheses(epoch.ID); if err != nil || !applied { t.Fatalf("applied=%v err=%v",applied,err) }
	for id, want := range map[string]float64{"thesis-eth":640,"thesis-link":560,"thesis-virtual":400} { got, err := db.ThesisCapitalLedgerByID(id); if err != nil || got.MaxExposureUSDT != want || got.RemainingDCAUSDT != want || got.Status != "ALLOCATED" { t.Fatalf("id=%s got=%+v err=%v",id,got,err) } }
	applied, err = db.ApplyDCAAllocationEpochToTheses(epoch.ID); if err != nil || applied { t.Fatalf("replay applied=%v err=%v",applied,err) }
	got, err := db.ThesisCapitalLedgerByID("thesis-eth"); if err != nil || got.MaxExposureUSDT != 640 { t.Fatalf("ledger=%+v err=%v",got,err) }
}

func TestApplyDCAAllocationEpochRejectsUnknownEpochAndConflictingLedger(t *testing.T) {
	db, err := Open(filepath.Join(t.TempDir(), "allocation.sqlite")); if err != nil { t.Fatal(err) }; defer db.Close()
	if _, err := db.ApplyDCAAllocationEpochToTheses(999); err == nil { t.Fatal("expected missing epoch") }
	epoch, _, err := db.CreateDCAAllocationEpoch(DCAAllocationEpochRequest{IdempotencyKey:"bootstrap", ObservedAvailableUSDT:2000, EnvelopeUSDT:1600, NetNewUSDT:1600, ObservedAt:time.Date(2026,7,24,8,0,0,0,time.UTC)}); if err != nil { t.Fatal(err) }
	if err := db.SaveThesisCapitalLedger(ThesisCapitalLedger{ThesisID:"thesis-eth",Symbol:"BTCUSDT",MaxExposureUSDT:1,RemainingDCAUSDT:1,Status:"ALLOCATED"});err != nil {t.Fatal(err)}
	if _, err := db.ApplyDCAAllocationEpochToTheses(epoch.ID); err == nil { t.Fatal("expected symbol conflict") }
	var applications int; if err := db.QueryRow(`SELECT COUNT(*) FROM dca_allocation_entry_applications`).Scan(&applications); err != nil || applications != 0 { t.Fatalf("applications=%d err=%v",applications,err) }
}
