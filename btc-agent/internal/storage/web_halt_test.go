...
func TestRequestWebHaltIsOneWayAuditedAndIdempotent(t *testing.T) {
	...
	r, e := d.RequestWebHalt("a@example.test", "Unsafe state found", "0123456789abcdef", now)
	if e != nil || r.Duplicate {
		t.Fatalf("%+v %v", r, e)
	}
	r, e = d.RequestWebHalt("a@example.test", "Unsafe state found", "0123456789abcdef", now.Add(time.Minute))
	if e != nil || !r.Duplicate {
		t.Fatalf("%+v %v", r, e)
	}
	h, e := d.IsHalted()
