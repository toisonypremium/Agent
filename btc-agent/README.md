...
- Code hỗ trợ production `live-auto`, nhưng quyền gửi lệnh thật chỉ là trạng thái runtime do operator kiểm soát qua control-plane/SQLite. Không tài liệu, env đơn lẻ, restart hoặc release nào được tự cấp quyền này. Audit gần nhất chỉ phê duyệt halted/canary-safe operation; chưa phê duyệt unrestricted live trading. Mỗi lệnh vẫn fail-closed qua market authority, audit/proof, final assertion, risk, reconcile, liquidity và notional caps; không gate nào được nới.
- Khi BTC chưa xác nhận accumulation, bot giữ `WATCH`/`SCOUT`, `desired=0`, không đặt lệnh.
- Dữ liệu futures/microstructure chỉ dùng quan sát, không dùng để thực thi futures.
...
- Web + SQLite là kênh vận hành chính; Supabase là cloud read model và R2 là kho artifact audit. Các kênh này không cấp execution authority.
- Telegram là fallback read-only tùy chọn. Không có lệnh mua/bán/hủy/override qua Telegram.
- LLM chỉ tạo narrative hoặc proposal bị ràng buộc; không override halt, doctor, reconciliation, ownership/fencing, stale-data, exchange filters hoặc capital locks. Xem `docs/llm-operations.md`.
- Survey, learning và report không được sửa config, không đặt lệnh, không bypass Agent 1/2 hoặc live guard.
...
Production chạy không có `--dry-run` không tự cấp quyền gửi lệnh. `APPROVED_MONITORING` hoặc `APPROVED_DRY_RUN` không phải quyền gửi lệnh thật. Trước mỗi thay đổi authority phải kiểm tra control-plane/runtime DB, ownership lease, reconciliation và audit hiện hành; không được suy diễn từ README, release, env hay service state.
...
`live-auto-audit` là command kiểm duyệt trước khi cho bot tự gửi lệnh thật. Nó ghi `reports/live_auto_audit_latest.md/json`, tách rõ current market authority và forced synthetic simulation, kiểm doctor/data/reconcile/risk/microstructure/proof/final assertion, và luôn kết luận rõ:
...
Telegram chỉ hiển thị state, blockers, trigger, orders, positions, doctor và supervisor. Không đặt/hủy/đóng lệnh và không override gates.
...
- Survey/learning evidence report-only.

### Milestone B: data sources

- Done: spot taker buy/sell volume.
...
- Done, report-only: liquidation proxy; không cấp thêm execution authority.
- Done, report-only: anchored VWAP/volume profile; chưa nối vào `ACTIVE_LIMIT` hoặc sizing.
...
- Done in code: embargoed, evaluation-only accumulation walk-forward report measures confirmed samples, false-positive rate and worst MAE.
- Pending evidence: compare against the approved baseline with enough real-data samples and complete manual review.
- Live sizing expansion remains disabled by policy; research verdicts never mutate config or execution authority.
- Exit/invalidation engine must remain proven before any future authority change.
