...
reconcile OKX, inspect halt and ownership, then monitor outbox. Never enable execution
without a current reconcile and valid fencing token. During cloud outage keep local
risk/reconcile running and let outbox retry. During OKX ambiguity halt new execution,
